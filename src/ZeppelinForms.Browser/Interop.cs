...
internal static partial class Interop
{
    ...
    // ==== JS -> C# ====
    // Окно ровно одно: в браузере canvas один, и хранить его в статике
    // проще, чем таскать идентификатор через каждый вызов из JS.

    internal static BrowserWindow? Window;

    [JSExport]
    internal static void OnFrame(double timestampMs) => Window?.HandleFrame(timestampMs);

    [JSExport]
    internal static void OnDrain() => Window?.DrainInvokes();

    [JSExport]
    internal static void OnResize(int physicalWidth, int physicalHeight, double scale) =>
        Window?.HandleResize(physicalWidth, physicalHeight, (float)scale);

    [JSExport]
    internal static void OnPointerMove(double x, double y, int modifiers) =>
        Window?.HandlePointerMove(x, y, modifiers);

    [JSExport]
    internal static void OnPointerDown(double x, double y, int button, int modifiers) =>
        Window?.HandlePointerDown(x, y, button, modifiers);

    [JSExport]
    internal static void OnPointerUp(double x, double y, int button, int modifiers) =>
        Window?.HandlePointerUp(x, y, button, modifiers);

    [JSExport]
    internal static void OnPointerLeave() => Window?.HandlePointerLeave();

    [JSExport]
    internal static void OnWheel(double x, double y, double deltaY, double deltaX) =>
        Window?.HandleWheel(x, y, deltaY, deltaX);

    [JSExport]
    internal static void OnContextMenu(double x, double y) => Window?.HandleContextMenu(x, y);
    ...
    internal static void OnKeyDown(string code, string key, int modifiers, bool isRepeat) =>
        Window?.HandleKeyDown(code, key, modifiers, isRepeat);

    [JSExport]
    internal static void OnKeyUp(string code, int modifiers) => Window?.HandleKeyUp(code, modifiers);

    [JSExport]
    internal static void OnFocusLost() => Window?.HandleFocusLost();

    [JSExport]
    internal static void OnVisibilityChange(bool visible) =>
        BrowserPlatform.Current?.HandleVisibilityChange(visible);

    [JSExport]
    internal static void OnPageHide() => BrowserPlatform.Current?.HandlePageHide();
